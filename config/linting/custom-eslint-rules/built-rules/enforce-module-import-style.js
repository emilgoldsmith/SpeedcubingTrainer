...
const errorMessages = Object.assign({ invalidSubdirectory: `Only test and test helper directories are allowed in internal`, testFile: `In test files you are only allowed to import the file you are testing, module types and any local helper files`, testHelperFile: `You are only allowed to do third party imports from a test helper file`, dependenciesFile: 'Dependency files are only allowed to do absolute imports', notRelativeSibling: `Internal directory files are only allowed to relatively import other files in that internal directory`, externalFile: `Only direct parent index files are allowed to import from internal directory` }, check_test_imports_1.getTestImportErrorMessages());
...
function checkInternalSubdirectory({ currentPath, importPath, }) {
    ...
    function isModuleException() {
        const isTestFileImportingTypes = currentPath.isInternalTestFile() && importPath.isParentTypesFile();
        const isHelperFileImportingInternal = currentPath.isTestHelperFile() && importPath.isGrandParent();
        return isTestFileImportingTypes || isHelperFileImportingInternal;
    }
}
function checkInternalTopLevel({ currentPath, importPath, }) {
    if (importPath.isFromReact())
        return null;
...
function checkExternal({ currentPath, importPath }) {
    ...
    function isNotIndexFileException() {
        const isIndexFileException = currentPath.isIndexFile() &&
            importPath.isChild() &&
            importPath.inInternal();
        return !isIndexFileException;
    }
}
module.exports = experimental_utils_1.ESLintUtils.RuleCreator(() => 'This rule is not public')({
