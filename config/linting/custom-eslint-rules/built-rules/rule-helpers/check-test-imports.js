...
const errorMessages = {
    __testImport__wrongImportFromTestFile: 'Test files are only allowed to relatively import their local helper files, and the file they are testing (which should have the same basename as the test). Anything else should either not need to be imported or be placed in a helper file',
    __testImport__wrongImportFromTestHelper: 'Test helper files are only allowed to import third party dependencies',
};
...
function checkTestImports({ currentPath, importPath, }) {
    ...
    function checkTestFile() {
        const isAllowed = importPath.isTestHelper() ||
