...
class FilePath extends Path {
    ...
    isInternalTestFile() {
        return (this.isInTestDirectoryUnder(this.names.internalDirectory) &&
    ...
    isInTestDirectoryUnder(grandparent) {
        ...
        if (grandparent)
            ancestors = [grandparent].concat(ancestors);
...
class ImportPath extends Path {
    ...
    isChild() {
        ...
        return (this.parser.start() === '.' && this.parser.length() === expectedLength);
    }
    ...
    isFromReact() {
        return (this.parser.allAncestorsAre([]) && this.parser.fileName() === 'react');
    }
    isImportingThisFilename(filename) {
        const matchesExactly = this.parser.fileName() === filename;
        const matchesWithImplicitExtension = this.parser.fileName() === filename.replace(/\.tsx?$/, '');
